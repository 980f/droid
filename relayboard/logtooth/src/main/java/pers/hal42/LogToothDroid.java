...
public class LogToothDroid extends GriddedActivity implements RfcommDevice.Snooper {
  ...
  public void onCreate(Bundle savedInstanceState) {
    ...
    gridManager.addButton("save list", new View.OnClickListener() {
      @Override
      public void onClick(View v) {
        saveCommandline();
      }
    });
    gridManager.addButton("list paired", new View.OnClickListener() {
      @Override
      public void onClick(View v) {
        getCached();
      }
    });
    gridManager.addButton("discover", new View.OnClickListener() {
      @Override
      public void onClick(View v) {
        doDiscovery();
      }
    });
    gridManager.addButton("!CLEAN", new View.OnClickListener() {
      @Override
      public void onClick(View v) {
        forgetAll();
      }
    ...
    gridManager.addButton("<-send that", new View.OnClickListener() {
      @Override
      public void onClick(View v) {
        sendCommand(cmdBox.getText().charAt(0));
      }
    });

    gridManager.addButton("0 Log", new View.OnClickListener() {
      @Override
      public void onClick(View v) {
        out.cls();
      }
    ...
    gridManager.addButton("0 Dev", new View.OnClickListener() {
      @Override
      public void onClick(View v) {
        devOut.cls();
      }
        ...
        try {
          ...
          if (intent!=null) {
            ...
            if (BluetoothDevice.ACTION_FOUND.equals(action)) {   // When discovery finds a device
              ...
              addDevice(device);
            } else if (BluetoothDevice.ACTION_UUID.equals(action)) {  //fetchUuidsWithSdp
              BluetoothDevice device = intent.getParcelableExtra(BluetoothDevice.EXTRA_DEVICE);
              if(device!=null) {//should never happen, but does.
                for (Parcelable uuid : intent.getParcelableArrayExtra(BluetoothDevice.EXTRA_UUID)) { //isa ParcelUuid
                  log.i(out.printf("sdp uuid %s\n%s\n", device.getAddress(), uuid == null ? "null uuid" : uuid.toString()));
                }
              } else {
                log.e("uuid action did not have device object");
              }
            }
          } else {
            log.d("null intent sent to our BroadcastReceiver!");
          }
        } catch (Exception e) {
          e.printStackTrace();
        }
...
//super fn is trivial:--    super.onActivityResult(requestCode, resultCode, data);
  ...
  private void connectOne(BluetoothDevice btDevice) {
    ...
    //find sensor
    for (RfcommDevice sensor : sensors) {
      if (sensor.wraps(btDevice)) {
        if (sensor.connectLogger(this)) {
          this.sensor = sensor;
